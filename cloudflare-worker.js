...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .container { position: relative; z-index: 10; width: 100%; max-width: 480px; }
    .card {
      background: rgba(255, 255, 255, 0.92);
      backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px);
      border: 2px solid #25385b; border-radius: 26px; padding: 30px 28px 24px;
    ...
    .logo-box {
      width: 70px; height: 70px; margin: 0 auto 14px; border-radius: 20px; background: #fff;
      border: 2px solid #25385b; display: flex; align-items: center; justify-content: center;
      box-shadow: 3px 3px 0px #25385b; font-size: 34px;
    }
    h1 {
    ...
    .btn-reload:active {
      transform: translate(4px, 4px); box-shadow: 0px 0px 0px #25385b;
    }
    .live-status {
      font-size: 11px; color: #94a3b8; margin-top: 14px; font-family: 'IBM Plex Mono', monospace;
    }
  ...
  <script>
    async function checkConnection() {
      ...
      try {
        ...
        if (res.ok) {
          liveHint.textContent = '🎉 Trạm phát nhạc đã bật! Đang chuyển hướng...';
          liveHint.style.color = '#10b981';
          setTimeout(() => location.reload(), 500);
          return;
        }
      } catch (e) {}
      setTimeout(() => {
        btnText.textContent = 'Trạm nhạc chưa bật, thử lại';
        btnIcon.textContent = '🔄';
        liveHint.textContent = 'Trạm nhạc vẫn đang tắt. Hãy thử lại sau nhé!';
      }, 800);
    }
    setInterval(async () => {
      try {
        const res = await fetch('/api/health', { cache: 'no-store', signal: AbortSignal.timeout(2000) });
